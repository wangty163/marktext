...
class CodeBlockContent extends Content {
    ...
    override inputHandler(event: Event): void {
        ...
        // zero-width caret anchors exist so the browser has a paintable insertion
        // point, and that character must never reach the code text. (A trailing
        // newline here is a `mu-trailing-break` <br>, which has no text of its
        // own, so the blacklist is what keeps this read safe in every shape.)
