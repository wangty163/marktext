...
describe('softLineBreak renderer — softNewlineAsSpace', () => {
    ...
    it('parks a caret anchor when isAtEnd is true and softNewlineAsSpace is off', () => {
        const renderer = asRenderer({
            muya: { options: { softNewlineAsSpace: false } },
        });
        const token = makeToken({ isAtEnd: true });
        ...
        // Not a block-level `mu-line-end` wrapper: with literal paragraph line
        // breaks the trailing `\n` already renders a line of its own, and a
        // second block box would claim one more (#5282). The caret instead gets
        // a zero-width anchor it can be painted in.
        expect(getSelector(out)).not.toContain(CLASS_NAMES.MU_LINE_END);
        expect(out).toHaveLength(2);
        expect(out[1].sel).toBe(`span.${CLASS_NAMES.MU_CARET_ANCHOR}`);
    });
});
