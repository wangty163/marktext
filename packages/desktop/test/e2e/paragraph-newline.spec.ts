...
test('prose below a heading keeps literal blank lines and Markdown formatting', async() => {
  ...
  try {
    ...
    // The source blank line between the heading and the prose is its own
    // editable block, so the prose paragraph is the second `.mu-paragraph`.
    await expect(page.locator('.mu-paragraph')).toHaveCount(2)
    const blank = page.locator('.mu-paragraph-content').first()
    const prose = page.locator('.mu-paragraph-content').nth(1)
    const lineHeight = await prose.evaluate(el => Number.parseFloat(getComputedStyle(el).lineHeight))
    // The blank line is painted as a real empty row, not swallowed.
    await expect.poll(() => blank.evaluate(el => el.getBoundingClientRect().height)).toBe(lineHeight)
    await expect.poll(() => paragraphText(page, 1)).toBe('alpha\n\n**beta**')
    await expect(prose.locator('strong')).toContainText('beta')
    const originalHeight = await prose.evaluate(el => el.getBoundingClientRect().height)
    await prose.click({ position: { x: 2, y: 5 } })
    await page.keyboard.press(lineEnd)
    await page.keyboard.press('Enter')
    await expect.poll(() => paragraphText(page, 1)).toBe('alpha\n\n\n**beta**')
    await expect.poll(() => prose.evaluate(el => el.getBoundingClientRect().height)).toBe(originalHeight + lineHeight)
    await saveWithKeyboard(app)
