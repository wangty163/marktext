...
// A plain-text document gets no markdown mode at all.
const sourceMode = () =>
  hasTextExtension(currentTab.value?.pathname || '') ? 'text/plain' : markdownMathMode()

watch([texMathDollars, texMathGfm, texMathSingleBackslash, texMathDoubleBackslash], () => {
  editor.value?.setOption('mode', sourceMode())
})
...
onMounted(() => {
  ...
  codeMirrorInstance.setOption('mode', sourceMode())
