...
// The badge carries the localized category name rather than a one-letter code,
// so it reads the same in every UI language. `all` is the all-characters count.
  ...
  t(
    {
      ...
      all: 'menu.counter.allCharacters'
    }[show.value]
  )
)
...
const wordCountText = computed(() => {
  ...
  return selectionValue == null
    ...
    : `${countLabel.value} ${value} / ${selectionValue}`
})
