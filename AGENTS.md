...
Use `scripts/install-local-macos.sh <e2e-spec>` from the repository root for this lifecycle. It checks the Playwright config and collects the requested tests before building or replacing the app; `--dry-run` performs that same preflight and only prints the remaining steps. Run a real install with host authorization because it writes `/Applications` and launches Electron; it builds, packages, keeps a recoverable non-`.app` backup, verifies the installed bundle and runs the requested installed-app E2E, restoring the previous app if any step fails.
